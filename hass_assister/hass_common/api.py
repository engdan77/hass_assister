...
class HassInstance(object):
    def __init__(
        ...
        if scheduler:
            scheduler.add_task(
                self.update_sensor_status, "interval", seconds=self.update_freq
            )
    ...
    async def update_sensor_status(self):
        ...
        try:
            async with aiohttp.ClientSession() as s, s.get(
                q, headers={"Authorization": f"Bearer {self.auth_token}"}
            ) as response:
    ...
    def get_entity_info(self, mqtt_topic: str) -> Dict:
        c = self.config
        config_lists = [
            c[root_key] for root_key in c.keys() if type(c[root_key]) is list
        ]
        valid_entities = [
            _dict
        ...
        return next(
            iter([e for e in valid_entities if e["state_topic"] == mqtt_topic]), None
        )
