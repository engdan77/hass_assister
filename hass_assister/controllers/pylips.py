...
parser = argparse.ArgumentParser(
    description="Control Philips TV API (versions 5 and 6)"
)
...
class Pylips:
    def __init__(self, ini_file):
        ...
        if len(sys.argv) > 1:
            ...
            if args.user and args.password:
                ...
                len(self.config["TV"]["user"]) == 0
                or len(self.config["TV"]["pass"]) == 0
            ) and self.config["TV"]["port"] == 1926:
                return print(
        ...
        if (
            len(sys.argv) == 1 or (len(sys.argv) == 3 and sys.argv[1] == "--config")
        ) and self.config["DEFAULT"]["mqtt_listen"] == "True":
    ...
    def check_if_paired(self):
        if str(self.config["TV"]["protocol"]) == "https://" and (
            len(str(self.config["TV"]["user"])) == 0
            or len(str(self.config["TV"]["pass"])) == 0
        ):
            return False
        else:
            return True
    ...
    def pair_request(self, data, err_count=0):
        ...
        auth["auth_signature"] = self.create_signature(
            b64decode(secret_key), str(auth_Timestamp).encode() + str(pin).encode()
        )
        ...
        while err_count < 10:
            ...
            try:
                ...
                r = session.post(
                    ...
                    verify=False,
                    auth=HTTPDigestAuth(
                        self.config["TV"]["user"], self.config["TV"]["pass"]
                    ),
                    timeout=2,
                )
                ...
                print(
                    "Username for subsequent calls is: "
                    + str(self.config["TV"]["user"])
                )
                print(
                    "Password for subsequent calls is: "
                    + str(self.config["TV"]["pass"])
                )
        ...
        while err_count < int(self.config["DEFAULT"]["num_retries"]):
            if verbose:
                print(
                    ...
                    str(self.config["TV"]["protocol"])
                    + str(self.config["TV"]["host"])
                    + ":"
                    + str(self.config["TV"]["port"])
                    + "/"
                    + str(self.config["TV"]["apiv"])
                    + "/"
                    + str(path),
            ...
            try:
                r = session.get(
                    ...
                    verify=False,
                    auth=HTTPDigestAuth(
                        str(self.config["TV"]["user"]), str(self.config["TV"]["pass"])
                    ),
                    timeout=2,
                )
            except Exception:
    ...
    def post(self, path, body, verbose=True, callback=True, err_count=0):
        while err_count < int(self.config["DEFAULT"]["num_retries"]):
            ...
            if (
                callback
                and self.config["DEFAULT"]["mqtt_listen"].lower() == "true"
                and len(sys.argv) == 1
            ):
            ...
            if (
                self.config["DEFAULT"]["mqtt_listen"].lower() == "true"
                and len(sys.argv) == 1
            ):
                self.mqtt_update_status(
                    {
        ...
        if command in self.available_commands["get"]:
            ...
            if "body" in self.available_commands["post"][command] and body is None:
                return self.post(
                    ...
                    self.available_commands["post"][command]["body"],
                    verbose,
                    callback,
                )
    ...
    def start_mqtt_listener(self):
        ...
        if (
            len(self.config["MQTT"]["user"]) > 0
            and len(self.config["MQTT"]["pass"]) > 0
        ):
        ...
        if self.config["MQTT"]["TLS"].lower() == "true":
            if len(self.config["MQTT"]["cert_path"].strip()) > 0:
                ...
                self.mqtt.tls_set()
        self.mqtt.connect(
            str(self.config["MQTT"]["host"]), int(self.config["MQTT"]["port"]), 60
        )
        if (
            self.config["DEFAULT"]["mqtt_listen"] == "True"
            and self.config["DEFAULT"]["mqtt_update"] == "False"
        ):
    ...
    def mqtt_update_powerstate(self):
        ...
        if powerstate_status is not None and powerstate_status[0] == "{":
            ...
            if "powerstate" in powerstate_status:
                if (
                    "powerstate" in self.last_status
                    and self.last_status["powerstate"]
                    != powerstate_status["powerstate"]
                ):
                    self.mqtt.publish(
                        str(self.config["MQTT"]["topic_pylips"]),
                        json.dumps(
                            {"status": {"powerstate": powerstate_status["powerstate"]}}
                        ),
                        retain=False,
                    )
        ...
        if ambilight_status is not None and ambilight_status[0] == "{":
            ...
            if "styleName" in ambilight_status:
                ...
                if json.dumps(self.last_status["ambilight"]) != json.dumps(ambilight):
                    self.mqtt.publish(
                        str(self.config["MQTT"]["topic_pylips"]),
                        json.dumps({"status": {"ambilight": ambilight}}),
                        retain=False,
                    )
        ...
        if ambihue_status is not None and ambihue_status[0] == "{":
            ...
            if "values" in ambihue_status:
                ...
                if self.last_status["ambihue"] != ambihue:
                    self.mqtt.publish(
                        str(self.config["MQTT"]["topic_pylips"]),
                        json.dumps({"status": {"ambihue": ambihue}}),
                        retain=False,
                    )
    ...
    def mqtt_update_app(self):
        ...
        if actv_status is not None and actv_status[0] == "{":
            ...
            if "component" in actv_status:
                if (
                    actv_status["component"]["packageName"] == "org.droidtv.zapster"
                    or actv_status["component"]["packageName"] == "NA"
                ):
                    self.mqtt_update_channel()
                else:
                    if (
                        self.last_status["cur_app"] is None
                        or self.last_status["cur_app"]
                        != actv_status["component"]["packageName"]
                    ):
                        self.mqtt.publish(
                            str(self.config["MQTT"]["topic_pylips"]),
                            json.dumps(
    ...
    def mqtt_update_channel(self):
        ...
        if channel is not None and channel[0] == "{":
            ...
            if "channel" in channel:
                if json.dumps(self.last_status["cur_app"]) != json.dumps(
                    {"app": "TV", "channel": channel}
                ):
                    self.mqtt.publish(
                        str(self.config["MQTT"]["topic_pylips"]),
                        json.dumps(
                            {"status": {"cur_app": {"app": "TV", "channel": channel}}}
                        ),
                        retain=False,
                    )
    ...
    def mqtt_update_volume(self):
        ...
        if vol_status is not None:
            ...
            if "muted" in vol_status:
                ...
                if (
                    self.last_status["muted"] != muted
                    or self.last_status["volume"] != volume
                ):
                    self.mqtt.publish(
                        str(self.config["MQTT"]["topic_pylips"]),
                        json.dumps({"status": {"muted": muted, "volume": volume}}),
                        retain=False,
                    )
